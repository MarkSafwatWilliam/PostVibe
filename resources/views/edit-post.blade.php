<x-layout>
    <main
        ...
        <div class="md:max-w-sm lg:max-w-md">
            <h1 class="text-primary text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold mb-4">
                Edit Your Post
            </h1>
            <p class="text-xl">
        ...
        <div class="w-full md:w-auto md:flex-grow">
            <form action="/edit-post/{{$post->id}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex flex-col gap-y-[1.75rem]">
                    ...
                    <!-- Image Input -->
                    <div>
                        <label for="image" class="text-primary">cover image</label>
                        <div class="relative group w-full h-48 rounded-md border-dashed border-2 border-gray-300 hover:border-primary transition-colors flex items-center justify-center cursor-pointer overflow-hidden">
                            ...
                            <div class="flex flex-col items-center gap-2 text-gray-400 group-hover:text-primary transition-colors">
                                ...
                                @if($post->image)
                                    <p class="text-xs text-gray-500">Current: {{basename($post->image)}}</p>
                                @endif
                            </div>
                    ...
                    <div>
                        <button type="submit" class="bg-primary w-full text-white py-2 px-4 rounded-md">
                            Save Changes
                        </button>
                    </div>
                </div>
